...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_dims, output_dims=1):
        ...
        ### Changed the architecture and value as CW2 Guidance required ###
        self.model = nn.Sequential(nn.Linear(input_dims, 1024), nn.LeakyReLU(0.2), nn.Dropout(0.3),
...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    fig.suptitle('Generator images at {}th epoch'.format(num_epoch))
    
    # Generate the images 
    data_fake = G_net(z_).cpu().detach().numpy()   
    
    # Costruct the images
    for (ax, image) in zip(axs.flat, data_fake):
...
if __name__ == '__main__':
    ...
    # Added to circumvent cloudfare protection bug: #################
    # Following from: https://github.com/pytorch/vision/issues/1938
