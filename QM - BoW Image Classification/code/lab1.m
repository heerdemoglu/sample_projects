...
% EECS797 Machine Learning for Visual Data Analysis (QMUL)
...
% VGG Group in Oxford: http://www.robots.ox.ac.uk/~vgg/sohttps:
% //login.mathworks.com/embedded-login/v2/login.html?locale=en_GB#ftware/
...
% Assume a and b are two vectors, the  Euclidean distance 
% function is EuclideanDistance.m 
%--------------------------------------code--------------------------------
%{%
clear;
load('data/global/all_features');
...
% set a random value and we intead to find 20 patch from both trn and tst
wordid = 15; %397; %78;
...
clear BoW;
k = 2;% set the k for k-nn algorithm 
...
for k = 1:num_c
    error_n(k) = ...
        num_pc-length(find(predict_label((k-1)*num_pc+1:num_pc*k)==k));
end 
err = error_n/num_pc;
...
for ci = 1:num_class
    for cj = 1:num_class
        confusion_matrix(ci,cj) = ...
            size(find(predict_label((ci-1)* ...
            num_test_1c+1:ci*num_test_1c,:)==cj),1)/num_test_1c;
    end 
end 
close all; figure;
...
idright = right_v(12);% set number you are about to visualize
idwrong = wrong_v(12);% set number you are about to visualize, be sure it is nor larger than length of wrong_v 
...
figure, 
subplot(2,4,1),subimage(imread(strcat('image/', ...
    image_names{idright+300}))),title('test image'),axis off;
subplot(2,4,2),bar(BoW(idright+300,:)),xlim([1,500]),...
    title('BoW of test image');
subplot(2,4,3),subimage(imread(strcat('image/',...
    image_names{NNresult(idright)}))), ...
    title('Right Label NN image'),axis off;
subplot(2,4,4),bar(BoW(NNresult(idright),:)),...
    xlim([1,500]),title('BoW of NN image');

subplot(2,4,5),subimage(imread(strcat('image/',...
    image_names{idwrong+300}))),title('test image'),axis off;
subplot(2,4,6),bar(BoW(idwrong+300,:)),xlim([1,500]),...
    title('BoW of test image');
subplot(2,4,7),subimage(imread(strcat('image/',...
    image_names{NNresult(idwrong)}))),...
    title('Wrong Label NN image'),axis off;
subplot(2,4,8),bar(BoW(NNresult(idwrong),:)),...
    xlim([1,500]),title('BoW of NN image');
%}
%-------------------------------end of code--------------------------------
...
% Libsvm is used: http://www.csie.ntu.edu.tw/~cjlin/libsvm/
...
test_data     = BoW(301:400,:);
clear BoW;
% set the parameters via cross-validation! Elapsed time is 246.92 seconds.
% bestc=64; bestg=2.2974;
% bestc = 1024; bestg = 2.82843;
bestcv=0;
tic 
for log2c = -1:10,
  for log2g = -1:0.1:1.5,
...
%{%
[predict_label, accuracy , dec_values] = svmpredict(test_labels,...
    test_data, model,'-b 1');
%}
...
error_n = zeros(num_c,1);
for k = 1:num_c
    error_n(k) = num_pc-length(find(predict_label((k-1)*...
        num_pc+1:num_pc*k)==k));
end 
err = error_n/num_pc;
err_all = sum(error_n)/(num_pc*num_c);
...
for ci = 1:num_class
    for cj = 1:num_class
        confusion_matrix(ci,cj)=size(find(predict_label((ci-1)*...
            num_test_1c+1:ci*num_test_1c,:)==cj),1)/num_test_1c;
    end 
end 
close all,
...
figure 
for k = 1:nv
    subplot(nrows,5,2*k-1),subimage(imread(strcat('image/',...
        image_names{right_v(k)+300}))),title(strcat('PL-',...
        class_names{predict_label(right_v(k))})),axis tight,axis off;
    subplot(nrows,5,2*k),subimage(imread(strcat('image/',...
        image_names{wrong_v(k)+300}))),title(strcat('PL-',...
        class_names{predict_label(wrong_v(k))})),axis tight,axis off;
end 
%}
%-------------------------------end of code--------------------------------