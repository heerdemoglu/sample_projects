...
class NeuralNetwork():
    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        #for i in range(self.n_out):
            ...
            # output_deltas[i] = (outputs[i]-targets[i]) * sigmoid_derivative(outputs[i]) # why loop here??? VECTOR IS EASY.
            ########################################/
        ...
        hidden_deltas = sigmoid_derivative(self.y_hidden) * np.sum(self.w_out * output_deltas)
        ...
        #for i in range(len(hidden_deltas)):
            ...
            #...
            #...
            #hidden_deltas[i] = sigmoid_derivative(inputs[i]) * ...
           # np.dot(self.w_out[i,:], output_deltas[i])
